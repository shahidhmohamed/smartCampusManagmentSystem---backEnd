...
export class ClassScheduleService {
  ...
  protected resourceUrl = this.applicationConfigService.getEndpointFor('api/class-schedules');
  protected resourceSearchUrl = this.applicationConfigService.getEndpointFor('api/class-schedules/_search');

  create(classSchedule: NewClassSchedule): Observable<EntityResponseType> {
    return this.http.post<IClassSchedule>(this.resourceUrl, classSchedule, { observe: 'response' });
  }

  update(classSchedule: IClassSchedule): Observable<EntityResponseType> {
    return this.http.put<IClassSchedule>(`${this.resourceUrl}/${this.getClassScheduleIdentifier(classSchedule)}`, classSchedule, {
      observe: 'response',
    });
  }

  partialUpdate(classSchedule: PartialUpdateClassSchedule): Observable<EntityResponseType> {
    return this.http.patch<IClassSchedule>(`${this.resourceUrl}/${this.getClassScheduleIdentifier(classSchedule)}`, classSchedule, {
      observe: 'response',
    });
  }

  find(id: string): Observable<EntityResponseType> {
    return this.http.get<IClassSchedule>(`${this.resourceUrl}/${id}`, { observe: 'response' });
  }
  ...
  search(req: SearchWithPagination): Observable<EntityArrayResponseType> {
    const options = createRequestOption(req);
    return this.http
      .get<IClassSchedule[]>(this.resourceSearchUrl, { params: options, observe: 'response' })
      .pipe(catchError(() => scheduled([new HttpResponse<IClassSchedule[]>()], asapScheduler)));
  }

  getClassScheduleIdentifier(classSchedule: Pick<IClassSchedule, 'id'>): string {
