...
export class ModuleComponent implements OnInit {
  ...
  trackId = (item: IModule): string => this.moduleService.getModuleIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && ModuleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ModuleComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.moduleService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
