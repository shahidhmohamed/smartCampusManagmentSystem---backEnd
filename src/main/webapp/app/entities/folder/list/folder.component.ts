...
export class FolderComponent implements OnInit {
  ...
  trackId = (item: IFolder): string => this.folderService.getFolderIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && FolderComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && FolderComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.folderService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
