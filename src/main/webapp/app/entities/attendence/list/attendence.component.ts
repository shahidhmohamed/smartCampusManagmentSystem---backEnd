...
import { AttendenceService, EntityArrayResponseType } from '../service/attendence.service';
...
export class AttendenceComponent implements OnInit {
  ...
  attendences = signal<IAttendence[]>([]);
  isLoading = false;
  ...
  protected readonly attendenceService = inject(AttendenceService);
  protected readonly activatedRoute = inject(ActivatedRoute);
  protected readonly sortService = inject(SortService);
  protected modalService = inject(NgbModal);
  protected ngZone = inject(NgZone);

  trackId = (item: IAttendence): string => this.attendenceService.getAttendenceIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && AttendenceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && AttendenceComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.attendences.set(dataFromBody);
  }

  protected fillComponentAttributesFromResponseBody(data: IAttendence[] | null): IAttendence[] {
    return data ?? [];
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.attendenceService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
    return this.attendenceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
