...
export class ChatComponent implements OnInit {
  ...
  trackId = (item: IChat): string => this.chatService.getChatIdentifier(item);
  ...
  search(query: string): void {
    ...
    if (query && predicate && ChatComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
  ...
  protected fillComponentAttributeFromRoute(params: ParamMap, data: Data): void {
    ...
    if (params.has('search') && params.get('search') !== '') {
      ...
      const { predicate } = this.sortState();
      if (predicate && ChatComponent.NOT_SORTABLE_FIELDS_AFTER_SEARCH.includes(predicate)) {
        this.sortState.set({});
      }
    }
  }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    if (this.currentSearch && this.currentSearch !== '') {
      return this.chatService.search(queryObject).pipe(tap(() => (this.isLoading = false)));
    }
